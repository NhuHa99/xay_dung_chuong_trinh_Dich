...
void compileForSt(void)
{
    ...
    eat(KW_FOR);
    eat(TK_IDENT);
    //FOR I := 1 TO 2 I, 1, 2 phai cung kieu
    // check if the identifier is a variable
    Object *var = checkDeclaredVariable(currentToken->string);
    eat(SB_ASSIGN);
    Type *exp1 = compileExpression();
    checkTypeEquality(var->varAttrs->type, exp1);
    eat(KW_TO);
    Type *exp2 = compileExpression();
    checkTypeEquality(var->varAttrs->type, exp2);
    eat(KW_DO);
    compileStatement();
}
...
void compileArguments(ObjectNode *paramList)
{
    ...
    case SB_LPAR:
        eat(SB_LPAR);
        if (lookAhead->tokenType == SB_RPAR || paramList == NULL)
        {
...
Type *compileFactor(void)
{
    ...
    case TK_IDENT:
        ...
        {
        case OBJ_CONSTANT:
            type = makeIntType(); //su dung nhu kieu rong vi chua xac dinh duoc kiue cua const
