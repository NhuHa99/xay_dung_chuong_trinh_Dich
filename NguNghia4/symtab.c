...
/******************* Object utilities ******************************/
//khởi tạo một phạm vi block mới
...
//khởi tạo đối tượng chương trình mới với tên có sẵn 
Object* createProgramObject(char *programName) {
  Object* program = (Object*) malloc(sizeof(Object));
  strcpy(program->name, programName);
  program->kind = OBJ_PROGRAM;
  program->progAttrs = (ProgramAttributes*) malloc(sizeof(ProgramAttributes));
  program->progAttrs->scope = createScope(program,NULL);//khởi tạo một phạm vi mới của ctrinh 
  symtab->program = program; //bảng ký hiệu lúc này chỉ có program 

  return program;
}
//khởi tạo một đối tượng hàm số mới 
Object* createConstantObject(char *name) {
  Object* obj = (Object*) malloc(sizeof(Object));
  strcpy(obj->name, name); //tên là name
  obj->kind = OBJ_CONSTANT;//kiểu là constant
  obj->constAttrs = (ConstantAttributes*) malloc(sizeof(ConstantAttributes));
  return obj;
}
//khởi tạo một đối tượng kiểu mới
...
//khởi tạo một đối tượng biến mới 
Object* createVariableObject(char *name) {
  Object* obj = (Object*) malloc(sizeof(Object));
  strcpy(obj->name, name);
  obj->kind = OBJ_VARIABLE;
  obj->varAttrs = (VariableAttributes*) malloc(sizeof(VariableAttributes));
  obj->varAttrs->scope = symtab->currentScope;//gán phạm vi sử dụng của biến ở phạm vi hiện tại đnag đọc được 
  return obj;
}
//khởi tạo function 
Object* createFunctionObject(char *name) {
  ...
  obj->funcAttrs->paramList = NULL;//gán danh sách tham số của hàm là rỗng 
  obj->funcAttrs->scope = createScope(obj, symtab->currentScope);//phạm vi truy cập của hàm
  return obj;
}
...
//thêm một đối tượng vào trong danh sách các đối tượng 
...
//sau khi khởi tạo chương trình phải chuyển vào block chính
void enterBlock(Scope* scope) {
  symtab->currentScope = scope;
}
//khi dịch một hàm hay thủ tục phải cập nhập giá trị của currentScope 
void exitBlock(void) {
  symtab->currentScope = symtab->currentScope->outer;
}
//khai báo đối tượng . đăng ký obj vào khối hiện tại của chương trình .
